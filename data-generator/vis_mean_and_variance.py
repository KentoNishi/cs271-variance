...
for key1 in keys:
    for key2 in keys:
        for key3 in keys:
            ...
            if len(items) < 4:
                continue
            sampled_items = random.choices(items, k=4)
            fig, axs = plt.subplots(2, 2)
            for (((p_1, (i_1, j_1)), (p_2, (i_2, j_2))), ax) in zip(sampled_items, axs.flatten()):
                # plt.cla()
                # plt.imshow(varianceTimeGrid, cmap='viridis', interpolation='bilinear')
                ...
                ax.scatter(origin[1], origin[0], c='r', edgecolors='white')
                ax.scatter(j_1, i_1, facecolor='b', marker="$A$", edgecolors='white')
                ax.scatter(j_2, i_2, facecolor='b', marker="$B$", edgecolors='white')
